...
public class OrdAndLineDB : BaseDB
{
    ...
    public List<product> GetOrderProducts(int orderId)
    {
        ...
        var q = dataContext.ExecuteQuery<product>("SELECT product.name, product.id, product.fat, product.protein, product.carbs, product.totalCalories FROM product JOIN orderLine ON orderLine.prodId = product.id JOIN ordAndLine ON ordAndLine.orderLineId = orderLine.id JOIN orders ON ordAndLine.orderId = orders.id WHERE orderId=" + orderId);
